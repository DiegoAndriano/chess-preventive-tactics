...
<template>
    ...
    <transition name="slide">
        <div
         v-if="tactic.modal.feedback"
         class="bg-stone-500/40 h-full w-full fixed top-0 z-10"></div>
    </transition>
    <transition name="slide">
        <form v-if="tactic.modal.feedback"
            class="z-50 fixed rounded-lg m-10 w-[320px] h-[500px] bg-white z-50 px-4 py-4 top-0 bottom-0 my-auto mx-auto left-0 right-0 border border-1 border-stone-200 shadow-sm flex flex-col dark:bg-stone-800 dark:border-stone-700">
            <div class="flex justify-end items-baseline py-4 gap-4">
                <p class="text-sm dark:text-stone-100">Thanks for taking the time!! Let us know what you think...</p>
                <button @click.prevent="tactic.toggleFeedbackModal" class="text-stone-700 dark:text-stone-100 cursor-pointer">
            ...
            <div class="flex flex-col h-full pb-8">
                ...
                <div class="flex justify-end flex-col items-end grow pt-4">
                    <button @click.prevent="submit"
                        ...
                        dark:bg-stone-950 dark:border-none dark:shadow-sm dark:text-stone-100">Submit
                        <IconEdit class="w-6 h-6 lg:w-auto lg:h-8 dark:text-stone-100"></IconEdit></button>
                </div>
            </div>
        </form>
</transition></template>
